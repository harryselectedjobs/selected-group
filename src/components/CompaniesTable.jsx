...
import {
  Building2, X, Plus, Pencil, Trash2, Save, XCircle, CheckSquare, Square
} from "lucide-react";
...
export default function CompaniesTable({ search }) {
  ...
  const fetchCompanies = async () => {
    ...
    try {
      ...
      if (totalPages === 1) {
        const firstRes = await fetch(`${API_BASE}/api/companies?page=1&limit=${limit}${searchParam}`);
      ...
      const res = await fetch(`${API_BASE}/api/companies?page=${backendPage}&limit=${limit}${searchParam}`);
...
const cleaned = (data.data || []).map((item) => ({
  ...
  useEffect(() => { setPage(1); setTotalPages(1); setCheckedIds([]); }, [search]);
...
const handleUpdate = async () => {
  try {
    ...
    const res = await fetch(`${API_BASE}/api/companies/${selectedItem.companyId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
...
const handleDeleteMultiple = async () => {
  ...
  const allChecked = companies.length > 0 && companies.every((c) => checkedIds.includes(c.id));
  const toggleAll = () => {
    if (allChecked) {
      setCheckedIds((prev) => prev.filter((id) => !companies.find((c) => c.id === id)));
    } else {
      const newIds = companies.map((c) => c.companyId).filter((id) => !checkedIds.includes(id));
              ...
              {companies.length === 0 ? (
                ...
                >
                  <td className="p-3" onClick={(e) => e.stopPropagation()}>
                    <button onClick={(e) => toggleCheck(item.companyId, e)} className="text-gray-400 hover:text-white transition-colors">
                  ...
                  <td className="p-3 text-gray-400">{item.industry || "—"}</td>
                  <td className="p-3" onClick={(e) => e.stopPropagation()}>
      ...
      {selectedItem && (
        <div className="fixed top-[60px] right-0 w-[400px] h-[calc(100vh-60px)] bg-[#0f0f0f] border-l border-white/10 flex flex-col z-50 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold truncate max-w-[220px]">
                {isEditing ? (editForm.name || editForm.domain || "Edit Company") : selectedItem.displayName}
              </h2>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            <Field label="Name" name="name" value={editForm.name} editing={isEditing} onChange={handleEditFormChange} />
            <Field label="Domain" name="domain" value={editForm.domain} editing={isEditing} onChange={handleEditFormChange} />
            <Field label="Website" name="website" value={editForm.website} editing={isEditing} onChange={handleEditFormChange} />
            <Field label="Description" name="description" value={editForm.description} editing={isEditing} onChange={handleEditFormChange} />
            <Field label="LinkedIn Page" name="linkedin_company_page" value={editForm.linkedin_company_page} editing={isEditing} onChange={handleEditFormChange} />

            <div className="text-[10px] uppercase tracking-widest text-gray-600 font-semibold pt-2 pb-1 border-b border-white/5">
              Location
            </div>
            <Field label="Address" name="address" value={editForm.address} editing={isEditing} onChange={handleEditFormChange} />
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="ZIP" name="zip" value={editForm.zip} editing={isEditing} onChange={handleEditFormChange} />
            </div>
            ...
            <Field label="Area of Work" name="area_of_work" value={editForm.area_of_work} editing={isEditing} onChange={handleEditFormChange} />
            <Field label="Technology Category" name="technology_category" value={editForm.technology_category} editing={isEditing} onChange={handleEditFormChange} />
            <Field label="Software Category" name="software_category" value={editForm.software_category} editing={isEditing} onChange={handleEditFormChange} />

            <div className="text-[10px] uppercase tracking-widest text-gray-600 font-semibold pt-2 pb-1 border-b border-white/5">
              Financials
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Employees" name="numberofemployees" value={editForm.numberofemployees} type="number" editing={isEditing} onChange={handleEditFormChange} />
              <Field label="Annual Revenue" name="annualrevenue" value={editForm.annualrevenue} type="number" editing={isEditing} onChange={handleEditFormChange} />
            </div>
          </div>

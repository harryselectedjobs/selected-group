...
Internal tool at `/crm` for managing companies, contacts, and email outreach sequences. Requires token-based auth via a Node backend at `https://www.selected.jobs/api/api/v1` (configured in [src/services/api.js](src/services/api.js)).
